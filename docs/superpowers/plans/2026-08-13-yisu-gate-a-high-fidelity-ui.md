...
- [ ] **Step 2:** 复制 P05 有数据页面结构，升级衣物图片卡、八分类横向标签、搜索入口、默认角色“我”和衣橱/添加衣物/我的三栏导航；中间使用 `60×60 pt` 圆形加号，卡片名称和第二行元数据均需完整可读。
...
- [ ] **Step 11:** 按 V3B 语言重制 P09 衣物详情并提交用户确认，覆盖真实衣物大图、重要信息层级、编辑与返回入口。
...
- [ ] **Step 13:** 用户确认后，将 `gateB1.status` 设为 `accepted`，记录日期、最终节点 ID 和用户确认摘要；README 和交付管理包中的 S2-01 更新为已验收、完成度 100%。
...
**Interfaces:**
- Consumes: Gate B2 组件；P05、P06、中间圆形加号添加衣物入口和我的基础页的已确认结构与跳转。
...
- [ ] **Step 4:** 完成中间圆形加号添加衣物入口和我的基础页；点击加号直接进入 P07，衣橱进入时默认选中。
- [ ] **Step 5:** 对卡片执行长名称和第二行元数据检查，保证不遮挡、不与下一行卡片重叠；定义截断行数和最小卡片高度。
- [ ] **Step 6:** 在 `08 Hi-Fi Prototype` 连接衣橱、搜索、分类、添加衣物和我的流程，并审计每个返回和取消目标。
