...
- P12「我的」：展示昵称、账号、协议、隐私和退出登录；退出登录不因未完成或失败修改而拦截。

### 2.2 自动保存触发规则

- 单选、多选、日期及层级选择字段：用户完成选择后立即提交。
- 名称、品牌、价格和备注等输入字段：停止输入约 600–800 毫秒，或输入控件失去焦点、键盘收起时立即提交。
- 照片：在照片确认页点击“使用这张照片”后提交替换。
- 页面不显示统一保存按钮，也不显示“放弃未保存的更改”弹窗。
- 保存失败不自动重试，不写入本地待同步队列；页面离开后不承诺保留未成功提交的字段修改。
...
| 待提交 | 文本输入处于 600–800 ms 防抖窗口 | 显示当前输入，不显示阻断层 | 可继续输入 |
| 保存中 | 请求已发出 | 显示“保存中…”及轻量进度图标 | 可继续编辑其他字段 |
| 已保存 | 服务端确认成功 | 短暂显示“已保存”，随后淡出 | 无需操作 |
| 保存失败 | 请求失败或超时 | 显示“保存失败” | 再次修改字段后重新提交 |

失败时当前页面可继续显示用户输入，但该值不视为已保存。若同一字段产生多次连续修改，界面与最终服务端值必须以最后一次成功提交的用户输入为准。
...
- 分类、季节、颜色、收纳位置、尺码、材质、风格点击后打开衣序选择面板；购买日期使用衣序自定义 6 行、周一开头日历，未来日期不可选。
- 选择面板的“取消”关闭面板且不产生更新；完成选择后立即更新本地页面并提交。
- P09 返回 P05 后，首页卡片只展示最近一次成功保存的数据；失败修改不建立跨页面待同步状态。
...
- 客户端按字段维护最新修改序号；仅最后一次有效响应可以结束该字段的保存中状态。
- 服务端使用更新时间、版本号或等效的乐观并发控制，避免旧数据静默覆盖新数据。
- 不同字段可独立保存，但页面级状态需要汇总所有字段的保存结果。

### 4.3 失败处理与页面离开

- 保存失败仅显示失败状态，不自动重试，也不写入本地待同步队列。
- 用户继续修改该字段时，按照正常触发规则发起新的保存请求。
- 页面离开或退出登录不因保存失败而拦截；重新进入后以服务端最近一次成功保存值为准。
- 不存在“待同步退出确认”状态。
...
- 单字段保存失败：其他已成功字段不回滚；失败字段显示“保存失败”，再次修改后重新提交。
- 多字段同时保存：状态汇总优先级为“保存失败”高于“保存中”高于“已保存”。
- 用户快速返回：首页只读取最近一次成功保存的数据，不显示待同步标记。
- App 被系统终止：未成功提交的字段修改不承诺恢复。
- 照片上传失败：继续显示旧服务器照片；本次会话保留新照片草稿供用户主动重试。
...
| AC-EDIT-03 | P09 正常加载 | 连续输入名称后停止约 600–800 ms，或使输入框失焦 | 只以最新输入为目标提交，成功后显示“已保存” |
| AC-EDIT-04 | 自动保存请求失败 | 继续浏览或返回 P05 | 显示“保存失败”；不自动重试、不建立待同步状态，P05 显示最近一次成功保存值 |
...
- 自动保存的空闲、待提交、保存中、已保存和保存失败状态均有设计和验收证据；正式流程不存在待同步状态。
