...
- 系统相册选图后的预览、衣物新增表单、创建反馈、详情字段级自动保存与双入口换图。
- 衣物详情直接编辑、字段级自动保存和删除；原独立编辑页 P10 合并归档。
- 加载、空白、错误、无搜索结果、按钮禁用、表单校验、创建中、自动保存六态、换图失败和删除确认。
...
- 离线队列、搭配业务、OOTD、分享、会员、支付、社区、虚拟试穿；允许保留已确认的“搭配”底部导航占位入口，但不实现搭配内容。
- TestFlight、App Store、运营后台或 Web 管理端页面。
- 未经要求自行扩展的四栏或更多底部导航。Gate A 使用已确认的三栏底部导航：衣橱、搭配、我的。
...
flowchart TD
    ...
    E --> F[搜索/筛选]
    E --> G[P09 衣物详情 / 直接编辑]
    G -->|修改选择字段| H[完成选择后立即自动保存]
    G -->|修改文本字段| I[停止输入约 800 毫秒或失焦后自动保存]
    H --> J{保存结果}
    I --> J
    J -->|成功| G
    J -->|失败| K[保留本地修改并提示重试]
...
| P08 | 新增衣物 | 是 | 是 | 默认、校验失败、创建中、创建失败、换图 |
| P09 | 衣物详情 / 直接编辑 | 是 | 是 | 空闲、待提交、保存中、已保存、保存失败、待同步、换图失败、加载失败 |
| P10 | 编辑衣物（已合并归档） | 历史稿 | 否 | 不开发、不进入正式原型、不提供热点 |
| P11 | 删除确认 | 是 | 是 | 确认、删除中、删除失败 |
| P12 | 全局错误反馈 | 是 | 否 | 非阻断提示、可重试提示、待同步退出提示 |
...
- 整个 App 主层级使用底部三栏导航：衣橱、搭配、我的；进入时默认选中“衣橱”。P05 的加载、空白、有数据、分类筛选和加载失败状态均显示该导航。
- 点击“搭配”进入暂未开放占位页，只说明后续可创建和管理搭配，不提供创建、选择衣物、保存等本轮范围外功能。
- 点击“我的”进入基础账号页，展示账号邮箱、当前衣橱“我 · 本人”、账号与安全、关于衣序 YISU 和退出登录。若存在待同步衣物修改，退出前提示“仍有衣物修改尚未同步”，提供“继续同步”和“放弃并退出”；待同步内容不得跨账号提交。
- P06 搜索及 P07–P09、P11 选图、录入、详情直接编辑、删除等子流程不显示底部导航；返回 P05 后恢复原选中状态。P10 已合并归档，不进入正式流程。
...
- 右下角提供明显但不遮挡内容的“添加衣物”浮动按钮或原生工具栏按钮。
...
- 导航标题“添加衣物”，左侧“取消”，右侧或底部主按钮“完成添加”；页面为纵向滚动长表单。
- 顶部大图右下角提供统一“换图”图标按钮，触控区域至少 44 × 44 pt。取消选图返回 P08 并保留表单与当前照片；确认照片后只更新新增表单，不提前创建正式衣物。
- 字段依次完整展开。重要信息为图片、名称、分类、季节、收纳位置、备注；其他信息为颜色、品牌、价格、尺码、购买日期、材质、风格。两组通过标题、留白、分割线或浅色背景形成层级，不折叠。
- 图片、名称、分类或季节缺失时“完成添加”按钮禁用；用户触发后也要在字段附近给出原因。
...
- 创建中锁定重复提交，显示“正在添加…”。
- 创建失败时保留已填写内容和图片，显示原因和“重试”。
- 创建成功 → P09，不出现 AI 识别或处理中步骤。

### P09 衣物详情

**目标**：在同一页面查看并直接维护衣物信息，减少进入独立编辑页和统一保存的额外操作。

**内容**：大图及 P08 已保存的全部档案字段；补充添加日期。右上角为更多菜单并放置“删除衣物”，不显示“编辑”或统一“保存”按钮。名称、品牌、价格、备注使用就地输入；分类、季节、颜色、收纳位置、尺码、购买日期、材质、风格点击后打开 P08 已定义的选择控件。

**规则**：

- 图片是页面第一视觉层级。
- 顶部大图右下角提供统一“换图”图标按钮，触控区域至少 44 × 44 pt。
- 选择型字段完成选择后立即提交；名称、品牌、价格、备注停止输入约 800 毫秒、失焦或键盘收起时提交。
- 页面级轻量反馈包括：空闲时不显示；保存中显示“保存中…”；成功短暂显示“已保存”；失败显示“保存失败，点击重试”；离开页面仍未完成时记录为“待同步”。
- 保存失败不得回滚用户当前看到的有效输入。同一字段连续修改时，以最后一次用户输入为准，旧请求的迟到响应不得覆盖新值。
- 用户返回 P05 时保留最新本地值；保存成功后首页卡片同步显示新名称、分类或照片。
- 换图取消返回 P09 并保留原照片；确认新照片后自动上传并保存，失败时继续显示原照片并提供重试。
- 两张信息卡只通过暖白背景、圆角、间距和分割线区分，不显示“重要信息”“其他信息”标题或二级标题。
...
|---|---|---|---|
| 空闲 | 无待提交修改 | 不显示保存文案 | 查看或编辑任意字段 |
| 待提交 | 文本输入处于约 800 毫秒防抖窗口 | 保留本地最新值，不阻断输入 | 继续输入或收起键盘 |
| 保存中 | 请求已发出 | 显示“保存中…”和轻量进度图标 | 可编辑其他字段 |
| 已保存 | 服务端确认成功 | 短暂显示“已保存”后淡出 | 无需操作 |
| 保存失败 | 请求失败或超时 | 显示“保存失败，点击重试” | 点击重试或继续修改 |
| 待同步 | 离开页面时仍有未完成修改 | 本地保留，联网或回到前台后自动重试 | 继续使用 App |

### P10 编辑衣物

P10 已于 2026-08-14 合并至 P09，不再作为正式页面或开发交付。现有 P10 线框图和原型仅保留历史归档标识，不允许从 P09 或其他正式页面进入，不再定义保存、取消、放弃更改或重新上传照片流程。
...
- App 主层级使用 iOS 三栏 Tab Bar：衣橱、搭配、我的；衣橱默认选中。子流程使用 NavigationStack 或模态呈现，不重复显示 Tab Bar。
...
| AC-PROTO-02 | 衣橱为空 | 选图、填写必填字段并点击“完成添加” | 到达衣物详情，返回首页后出现新卡片 |
| AC-PROTO-03 | 衣橱已有数据 | 搜索名称并选择品类 | 结果正确，无结果时可一键清空条件 |
| AC-PROTO-04 | 进入衣物详情 | 直接编辑名称并停止输入约 800 毫秒 | 自动保存；详情和首页卡片同步展示新名称 |
| AC-PROTO-05 | 进入衣物详情 | 删除、取消，再次删除并确认 | 取消时数据不变；确认后返回首页且卡片消失 |
| AC-PROTO-06 | 登录、创建或自动保存失败 | 点击重试 | 已输入信息保留，页面提供可理解的恢复路径 |
| AC-EDIT-01 | P05 有衣物卡片 | 点击卡片 | 进入 P09，可直接编辑字段，不出现“编辑”按钮 |
| AC-EDIT-02 | P09 正常加载 | 修改选择型字段 | 选择完成后立即出现保存反馈并成功更新 |
| AC-EDIT-03 | P09 正常加载 | 连续输入名称后停止约 800 毫秒 | 只以最新输入为目标提交，成功后显示“已保存” |
...
| AC-SYNC-01 | P09 有待同步修改 | 从“我的”退出账号 | 显示待同步提示；修改不得跨账号提交 |
...
6. 衣物详情直接编辑、字段级自动保存、换图、删除和删除确认；P10 仅作历史归档。

必须包含正常、加载、空白、错误、无搜索结果、表单校验、禁用、创建中、保存中、已保存、保存失败、待同步、换图失败和删除失败等状态。P08 使用“完成添加”；P09 不得出现“编辑”或统一“保存”按钮。请生成用户流程图、全部低保真页面、关键页面高保真、最小 UI 规范和可点击交互。
...
禁止加入：Sign in with Apple、微信登录、手机号登录、相机拍摄、AI 识别、去背景、离线队列、搭配业务、OOTD、分享、会员、支付、社区、虚拟试穿、Web 管理端和四栏或更多底部导航。允许已确认的三栏导航，其中“搭配”仅为暂未开放占位。
