...
| v1.2 | 2026-08-25 | 冻结 P05 导航、P12 页面职责与 P09 直接编辑 | 消除旧“搭配”、P10 独立编辑、全局错误 P12 和待同步退出等冲突口径 |

### 0.1 Gate A 冻结规则优先级

以下规则覆盖本文后续仍保留的历史描述：P05 主导航固定为 `衣橱｜添加衣物｜我的`；P09 文本字段停止输入约 600–800 ms 后保存、失焦立即提交，保存失败不自动重试、不本地暂存、不产生待同步状态；P10 仅作历史归档；P12 固定为“我的”；换图失败继续显示旧服务器图片，并在当前会话保留新图片草稿供用户主动重试。
...
| 购买日期 | 否 | 衣序自定义 6 行日历（周一开头、未来日期禁用） | 2026-08-08 |
| 材质 | 否 | 多选列表 | 麻、棉 |
| 风格 | 否 | 多选列表 | 简约、通勤 |
...
| 保存失败 | 字段或图片提交失败 | 保留当前会话内容并提示，不自动重试、不建立本地待同步队列 | 主动重试或继续使用 App |
