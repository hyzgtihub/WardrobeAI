...
规范优先级为：已确认产品需求与交互规则 → 本文档 → `tokens.json` → Figma 高保真组件 → 页面实例。发现冲突时不得静默修改下游资产，应先修订规范版本，再同步 Figma 与代码。
...
2. 温和：以奶油白、暖白和柔焦灰调彩色代替冰冷纯白与沉闷冷灰。
...
- `background`：全局奶油白画布。
- `surface`：暖白卡片、输入框、浮层和导航。
- `brand / brandEmphasis / brandSubtle`：薰衣草主色、交互强调和浅选中态。
- `accentBlue / accentBlush / accentSage`：衣物图片区和辅助标签的低饱和背景；不表示业务状态。
- `textPrimary / textSecondary / textPlaceholder`：三级文本层级。
- `success / danger`：仅用于真实状态，不用于装饰。
- `border`：分割和控件边界，避免大面积深色描边。
...
- Display：品牌展示，仅用于登录品牌或专题入口。
- Large Title：页面主标题，例如“我的衣橱”。
- Title 1 / Title 2：内容标题和卡片标题。
- Headline：按钮、标签强调和关键值。
- Body / Callout：正文、表单值和主要说明。
- Subheadline / Footnote / Caption：元数据、帮助文案和底部导航。
...
- 输入框与小卡片：16 pt；内容卡片：20 pt；衣物图与大卡片：24 pt；底部导航与大容器：32 pt。
- 胶囊、圆形按钮统一使用 `radius.full`。
- 默认描边为 1 pt `border`；焦点态使用品牌色，不叠加多层轮廓。
- 阴影分为 `subtle`、`card`、`floating` 三层。不得把阴影用于每个列表行，也不得用高不透明黑影制造廉价悬浮感。
...
用途：主要、次要与破坏性操作。状态：Default、Pressed、Disabled、Loading。尺寸：Large 60、Medium 48；最小触控区 44 pt。一个页面通常只保留一个主要按钮。

### 10.2 YISU Hi-Fi/Icon Button

用途：返回、搜索、更多、密码可见性等单图标操作。状态：Default、Pressed、Disabled；尺寸 44 和 48。必须配置无障碍标签。

### 10.3 YISU Hi-Fi/Text Field

用途：邮箱、密码、名称、品牌、价格等输入。状态：Empty、Filled、Focused、Error、Disabled；可选前后图标和帮助文本。默认高度 56 pt，错误态需同时给出文字原因。
...
用途：衣橱两列网格。状态：Default、Pressed、Loading、Image Error；包含真实衣物图、名称和次级元数据。卡片点击进入 P09 直接编辑详情。

### 10.8 YISU Hi-Fi/Editable Field Row

用途：P09 进入即编辑的字段行。状态：Idle、Pressed、Saving、Saved、Error、Offline；支持单选、多选、文本、日期与层级选择入口。选择类字段变更后立即保存，文本字段停止输入后约 800 ms 提交。

### 10.9 YISU Hi-Fi/Photo Hero

用途：P08/P09 顶部衣物主图。状态：Loaded、Loading、Error；右下角始终提供 84×44 pt“换图”入口。取消选图返回原始入口页面。
...
用途：MVP 全局导航。状态：Wardrobe Selected、Profile Selected；中间固定为添加衣物主操作，不是 Tab。选中状态通过图标、文字和柔和彩底共同表达。
...
- 变更顺序：修订需求/交互依据 → 更新 `tokens.json` 与本文件 → 校验 → 更新 Figma → 更新 SwiftUI → 页面回归。
